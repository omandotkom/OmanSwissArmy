...
const md5 = (inputString: string) => {
    ...
    function rh(n: number): string { let j; let s = ""; for (j = 0; j <= 3; j++) s += hc.charAt((n >> (j * 8 + 4)) & 0x0F) + hc.charAt((n >> (j * 8)) & 0x0F); return s; }
    function ad(x: number, y: number): number { const l = (x & 0xFFFF) + (y & 0xFFFF); const m = (x >> 16) + (y >> 16) + (l >> 16); return (m << 16) | (l & 0xFFFF); }
    function rl(n: number, c: number): number { return (n << c) | (n >>> (32 - c)); }
    function cm(q: number, a: number, b: number, x: number, s: number, t: number): number { return ad(rl(ad(ad(a, q), ad(x, t)), s), b); }
...
export default function HashGenerator() {
    ...
    useEffect(() => {
        const generate = async () => {
            ...
            const sha1Buf = await crypto.subtle.digest('SHA-1', msgBuffer);
            const sha1Hash = Array.from(new Uint8Array(sha1Buf)).map(b => b.toString(16).padStart(2, '0')).join('');

            const sha256Buf = await crypto.subtle.digest('SHA-256', msgBuffer);
            const sha256Hash = Array.from(new Uint8Array(sha256Buf)).map(b => b.toString(16).padStart(2, '0')).join('');

            const sha512Buf = await crypto.subtle.digest('SHA-512', msgBuffer);
            const sha512Hash = Array.from(new Uint8Array(sha512Buf)).map(b => b.toString(16).padStart(2, '0')).join('');
            ...
            <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full">
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <textarea
                        ...
                        className="h-[100px] w-full resize-none rounded-xl border border-zinc-800 bg-zinc-900 p-4 font-mono text-sm text-zinc-300 focus:outline-none focus:border-zinc-600"
                    ...
                    {Object.entries(hashes).map(([algo, hash]) => (
                        <div key={algo} className="flex flex-col gap-2">
                            <div className="flex items-center justify-between">
                                <h2 className="text-sm font-medium text-zinc-400 uppercase">{algo}</h2>
                                <button
                            ...
                            <div className="rounded-lg bg-zinc-900 border border-zinc-800 p-3 font-mono text-sm text-zinc-300 break-all">
                                {hash || <span className="text-zinc-600 italic">Waiting for input...</span>}
                            </div>
                        </div>
                    ))}
                </div>
